...
export function useWeatherData(lat: number | null, lon: number | null) {
	...
	const forecast = useQuery({
		queryKey: ['forecast', lat, lon, temperatureUnit, windUnit, precipUnit],
		enabled,
		queryFn: async () => {
			const hourly = [
				'weather_code','is_day',
			...
			const daily = [
				'temperature_2m_max','temperature_2m_min','precipitation_sum','precipitation_probability_max','sunrise','sunset','uv_index_max','wind_speed_10m_max'
			].join(',');
			const current = [
