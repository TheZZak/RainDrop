...
import { formatDayLabel, formatTemp } from '@/lib/formatters'; // ← Add formatTemp here
import { usePreferences } from '@/store/usePreferences'; // ← Make sure this is imported
import {
	AirQualityCard,
	WindCard,
	PrecipitationCard,
...
const App = () => {
	...
	const { temperatureUnit } = usePreferences(); // ← Add this line
	...
	const temp = current?.temperature_2m != null ? formatTemp(current.temperature_2m!, temperatureUnit) : '—';	const desc = current?.weather_code != null ? weatherCodeToText(current.weather_code, (current.is_day ?? 1) === 1) : 'Loading…';	const IconToday = iconForWeatherCode(current?.weather_code ?? 1, (current?.is_day ?? 1) === 1);
	...
	// Fallback to hourly data at noon
	...
	return (
		<div className="min-h-screen text-white">
			<div className="relative z-10 mx-auto max-w-[1440px] px-8 py-8">
				...
				<div className="mt-2 grid grid-cols-[320px_minmax(0,1fr)_420px] gap-8 items-start">
					...
					{(daily?.time && daily.time.length) ? (
						<div className="divide-y divide-white/10">
							{daily.time.slice(0, 10).map((t, i) => (
								<ForecastRow key={i} dayLabel={formatDayLabel(t, i)} Icon={dailyIconAt(i)}
											 min={daily.temperature_2m_min?.[i] ?? 0}
											 max={daily.temperature_2m_max?.[i] ?? 0} minAll={minAll} maxAll={maxAll}
